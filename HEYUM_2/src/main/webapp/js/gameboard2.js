/*function showdicetable(){
	let html = '<img height="70px" id="dice1" alt="" src="../img/game/white.png"><img height="70px" id="dice2" alt="" src="../img/game/white.png"></div>'+
				'<div class="row"><button id="dicebtn" class="btn" onclick="throwDice()">주사위</button><button class="btn" onclick="back()">뒤로3칸이동</button>'+
				'<input type="text" value="0" id="inputdice">'+
				'<button onclick="turnend()">턴 종료</button></div>';

	$("#dicetable1").html(html);
}*/
/*
function throwDice(){
	$("#dicebtn").attr("disabled", true);
	let selfdice = parseInt($("#selfdice").val());
	if(selfdice==0){
		let dice1 = Math.floor(6 * Math.random()) + 1;
		let dice2 = Math.floor(6 * Math.random()) + 1;
		document.getElementById("dice1").src = "/HEYUM/img/game/dice"+dice1+".gif";
		document.getElementById("dice2").src = "/HEYUM/img/game/dice"+dice2+".gif";
		let sum = dice1+dice2;
		setTimeout(function() {
		 	move(1,sum);
		}, 1200);
